...
void printList(DynArr *heap)
{
	...
	DynArr *tempHeap = createDynArr(maxSize);
	Task *temp;
    for(int i = 0; i < sizeDynArr(heap); i++)
    {
    	temp = getDynArr(heap, i);
    	addDynArr(tempHeap, temp);
    }
    sortHeap(tempHeap);
    for(int i = sizeDynArr(tempHeap); i > 0; i--)
    {
    	print_type(getDynArr(tempHeap, i-1));
    }
...
/* Compare two tasks by priority by type casting the value of void* to the
 * desired type.
