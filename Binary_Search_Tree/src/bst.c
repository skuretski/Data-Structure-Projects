...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	if (cur == 0) {
	// Set up the current node and initialize the value to newValue
		...
		// Otherwise, if newValue is less than the current value...
		if (compare(val, cur->val) == -1) {
			cur->left = _addNode(cur->left,val);
		}
		// Otherwise, if newValue is greater than or equal to the current value...
		else {
